...
class CSPLMNNPipeline(BaseEstimator, ClassifierMixin):
    ...
    def evaluate(
        ...
        # Fit CSP (and LMNN) once; then run classifier grids on the fixed
        # 2D feature space to avoid 3D array validation inside GridSearchCV.
        csp = CSP(
            n_components=self.m_filters,
            reg="empirical",
            log=True,
        ...
        if self.use_lmnn:
            ...
            X_train_feat = lmnn.fit_transform(X_train_csp, y_train)
            X_test_feat = lmnn.transform(X_test_csp)
        else:
            X_train_feat = X_train_csp
        ...
        def run_grid(
            ...
            y_train_pred = best_est.predict(X_train_feat)
            y_test_pred = best_est.predict(X_test_feat)
        ...
        # AdaBoost with cross-validated hyperparameter search
        ada_res = run_grid(
            "ada",
